...
const float E_CMS_TMP = 5020; // Feb 2026
...
const float sizeForXY_for_CoreCorona = 11.5; // fm // Added in May 2018
const int nBinsXY_for_CoreCorona = 23;       // May 2018, after discussion with EA

float kCoeffRadiusForMB = 1.75; // tuned to be Ok for MB! (for NN)
...
NucleiCollision::NucleiCollision(/*int seed*/) : fNucleusType(nucleus_Pb), fNumberOfNucleons(208), // Pb nucleus
                                                 fMaxPartons(35000),                               //(8000),
                                                 fNucleusWSa(1.), fNucleusRadius(1.),              // 6.413)//5.5) //0.877) //radius proton, fm
                                                 //  , fRadiusParton(0.1)    //radius "parton", fm
                                                 fPartonInteractionDistance(0.2), // 0.2)
                                                 fFlagUsePoissonianNpartonsFluctuations(true), fFlagOnlyOneInteractionPerParton(true), fFlagPartonDestiny(0),
                                                 fFlagUsePartonPDFs(false), fMeanNofPartonsInNucleon(5), fNucleonGaussianRadius(0.4),
                                                 //  , fHardScatteringProbability(0.03)
                                                 fAvMultFromOneStringPerEtaUnitFictive(1.1), // 1.1 particle per unit of rapidity
                                                 fMultFromAllStringsFictiveV0(0), fMultFromAllStringsFictiveMidEta(0),
                                                 fImpactParameter(0.), // fm (is MC-sampled)
                                                 //  , fImpactParameterByHand_0_100(0.)
                                                 //  , fImpactParameter_rangeMin_0_100(0)
                                                 //  , fImpactParameter_rangeMax_0_100(100)
                                                 fImpactParameterByHand(0.), fImpactParameter_rangeMin(0.), fImpactParameter_rangeMax(1.), fImpactParSpecification(0), // MB
...
void NucleiCollision::initDataMembers()
{
    ...
    case nucleus_p_Pb:
        ...
        break;
    default:
        cout << "AHTUNG! fNumberOfNucleons: something is wrong!" << endl;
        break;
    }
    ...
    funcNucleonDensity = new TF1("funcNucleonDensity", "4.*3.1415926*x*x / (1 + TMath::Exp( (x-[0]) / [1] ))", 0, fNucleusRadius * 2);
    ...
    funcPartonDensity = new TF1("funcPartonDensity", "4.*3.1415926*x*x / (1 + TMath::Exp( (x-[0]) / [1] ))", 0, kNucleonSize * 2);
    ...
    fHistNpartonsPerNucleon = new TH1D("fHistNpartonsPerNucleon", "n partons per nucleon;N;entries", 400, -0.5, 400 - 0.5);
    ...
    fHist2DNstringsXY_thisEv_CoreCorona = new TH2I("fHist2DNstringsXY_thisEv_CoreCorona", "nStr_xy_this_for_CoreCorona", nBinsXY_for_CoreCorona, -sizeForXY_for_CoreCorona, sizeForXY_for_CoreCorona, nBinsXY_for_CoreCorona, -sizeForXY_for_CoreCorona, sizeForXY_for_CoreCorona);
    ...
    fHistNcoll = new TH1D("fHistNcoll", ";N_{coll};n events", 20 * 2 * fNumberOfNucleons + 1, -0.5, 20 * 2 * fNumberOfNucleons + 0.5);
    ...
    // ### PDFs - Jan 2023:
    funcGluon = new TF1("funcGluon", " [0] * TMath::Power( x, -0.2 ) * TMath::Power( 1-x, 5.4 )", 0.0001, 0.99999);
    // funcGluon = new TF1("funcGluon", " [0] * TMath::Power( x, -1.2 ) * TMath::Power( 1-x, 5.4 )", 0.001, 0.99999);
    funcGluon->SetNpx(2000);
    funcGluon->SetParameter(0, 1);

    funcU = new TF1("fU", " [0] * TMath::Power( x, 0.7 ) * TMath::Power( 1-x, 3.5 )", 0.0001, 0.99999);
    // funcU = new TF1("fU", " [0] * TMath::Power( x, -0.3 ) * TMath::Power( 1-x, 3.5 )", 0.001, 0.99999);
    funcU->SetParameter(0, 1);
    funcU->SetNpx(2000);

    funcD = new TF1("fD", " [0] * TMath::Power( x, 0.7 ) * TMath::Power( 1-x, 5.2 )", 0.0001, 0.99999);
    // funcD = new TF1("fD", " [0] * TMath::Power( x, -0.3 ) * TMath::Power( 1-x, 5.2 )", 0.001, 0.99999);
    funcD->SetParameter(0, 1);
    funcD->SetNpx(2000);

    funcSea = new TF1("fSea", " [0] * TMath::Power( x, -0.2 ) * TMath::Power( 1-x, 7 )", 0.0001, 0.99999);
    // funcSea = new TF1("fSea", " [0] * TMath::Power( x, -1.2 ) * TMath::Power( 1-x, 7 )", 0.001, 0.99999);
    ...
    // ### parton scattering angle: (Feb 2026: NOT USED?)
    ...
    const int ptz_nBins = 200;
    double ptz_startValueX = 0.2;
    double ptz_stopValueX = 2500;
    double ptz_arr_bins[ptz_nBins + 1];
    for (int i = 0; i <= ptz_nBins; i++)
        ptz_arr_bins[i] = pow(10, log10(ptz_startValueX) + (log10(ptz_stopValueX) - log10(ptz_startValueX)) / double(ptz_nBins) * double(i));

    // Feb 2026: monitor transverse mom of rescattered partons
    fHist_partons_pT_QA = new TH1D("fHist_partons_pT_QA", ";parton p_{T};n", 1000, 0, 100);
    // fHist_partons_pZ_vs_pT_QA = new TH2D("fHist_partons_pZ_vs_pT_QA", ";parton p_{z};parton p_{T}", 2500, 0, 2500, 400, 0, 20);
    fHist_partons_pZ_vs_pT_QA = new TH2D("fHist_partons_pZ_vs_pT_QA", ";parton p_{z};parton p_{T}", ptz_nBins, ptz_arr_bins, 400, 0, 20);
    fHist_partons_pX_pY_QA = new TH2D("fHist_partons_pX_pY_QA", ";parton p_{x};parton p_{y}", 400, -20, 20, 400, -20, 20);
    fHist_partons_individual_kick_dMom_X_Y_QA = new TH2D("fHist_partons_individual_kick_dMom_X_Y_QA", ";parton kick p_{x};parton kick p_{y}", 200, -10, 10, 200, -10, 10);
    fHist_partons_dx_dy = new TH2D("fHist_partons_dx_dy", ";parton pair dx;parton pair dy", 200, -5, 5, 200, -5, 5);
    fHist_partons_kick_theta_QA = new TH1D("fHist_partons_kick_theta_QA", ";#theta;", 400, 0, 8);
    fHist_partons_kick_cosTheta_QA = new TH1D("fHist_partons_kick_cosTheta_QA", ";cos(#theta);", 200, -1, 1);
    fHist_partons_kick_phi_QA = new TH1D("fHist_partons_kick_phi_QA", ";#phi;", 600, -10, 20);

    fHist_partons_x = new TH1D("fHist_partons_x", ";parton x;entries", 2000, 0, 1);

    fNWoundsPerNucleon = new TH1D("fNWoundsPerNucleon", ";n_{interactions};", 200, -0.5, 199.5);
    fNWoundsPerParton = new TH1D("fNWoundsPerParton", ";n_{interactions};", 1001, -0.5, 1000.5);

    // Feb 2026: add Theta rescattering angle vs impact par and mom
    // fThetaPartonRescattering = new TF2("fThetaPartonRescattering", "2*atan( [0]*0.197 / (2*x*y) )", 0.001, 2); // x = parton-parton impact par, y = parton momentum
    // fThetaPartonRescattering->SetNpx(1000);
    // fThetaPartonRescattering->SetParameter(0, 0.2); // alpha_s
}
    ...
    if (fFlagHaveMBcollision)
    {
        ...
        for (int iP = 0; iP < fA->nPartons; iP++)
        {
            ...
            if (1)
            {
                ...
                if (partState == 1)
                    fHistBusyPartonsR->Fill(rParton);
                if (partState == -1 || partState == 2)
                    fHistPartonsValenceR->Fill(rParton); //"valence quark"
                if (partState == 2)
                    fHistBusyPartonsValenceR->Fill(rParton); //"valence quark" is BUSY
            }
        }
...
void NucleiCollision::createPartons(Nucleus *nucl, int nId) // float nucleonX, float nucleonY, float *x, float *y )
{
    ...
    else // USE PDFs
    {
        ...
        while (xRemnant > 0.001)
        {
            ...
            nucl->pxBjorken[curN + partonCounter] = x;
            ...
            // if (partonCounter >= 20) // hand-made param, March 2026!
            // break;
    ...
    for (int iP = 0; iP < nPartonsInThisNucleon; iP++)
    {
        ...
        if (0) // variant with Gaus2D position of partons // was ON until March 2026!!!!
        {
        ...
        else if (1) // March 2026: try "hotter core" for partons
        {
            fRand->Rannor(a, b);                                             // mean=0 and sigma=1
            float kNucleonGausSigma = iP < 3 ? 0.8 : 0.6;//fNucleonGaussianRadius; // March 2026: try different radii for val. q vs the rest! // 0.4 - WAS A DEFAULT BEFORE 27 NOV 2017; //0.6

            if (iP > 3) // March 2026
            {
                // addintional modification using x Bjorken of parton - to make "hotter core" for partons with higher x
                if (nucl->pxBjorken[iP] > 0.05)
                    kNucleonGausSigma = 0.4;
                // else if (nucl->pxBjorken[iP] > 0.05)
                // kNucleonGausSigma = 0.6;
                else
                    kNucleonGausSigma = 0.6;//fNucleonGaussianRadius;
            }
    ...
    fHistNpartonsPerNucleon->Fill(nPartonsInThisNucleon);
}
...
{
    // cout << "we are in createStrings" << endl;
    ...
    if (fFlagPartonDestiny == 0) // default! "burning-out" partons or "wrong" mode of WQM used for QM2018 poster
    {
        // find the closest parton pairs
        fPartonInteractionsFinder->FindMinDistancesBetweenPairs(fA->pX, fA->pY, fB->pX, fB->pY, fA->nPartons, fB->nPartons, fFlagOnlyOneInteractionPerParton); // fX1,fY1,fX2,fY2,fMaxPartons,fMaxPartons);
        ...
        for (int iP = 0; iP < distArraySize; iP++)
        {
            ...
            float partonPairDist = sqrt(arrDist[iP].dist);

            // form a string between the two partons closest in transv. plane (should be used with flagOnlyOneInteractionPerParton==1)
            fXstring[fNumberOfStrings] = (fA->pX[id1] + fB->pX[id2]) / 2.;
            fYstring[fNumberOfStrings] = (fA->pY[id1] + fB->pY[id2]) / 2.;
            // cout << "id1=" << id1 << ", id2=" << id2 << endl;
            fDistanceBetweenPartonsForString[fNumberOfStrings] = partonPairDist;

            // string origin (Sept 2017)
            short stringOrigin = 0; // sea quarks/gluons
            if (0)
                cout << "status of partons for string id " << fNumberOfStrings << ": "
            ...
            fHistMinDistBetweenInteractingPartons->Fill(partonPairDist);
            if (stringOrigin == 2) // interaction b/n valence quarks
                fHistMinDistBetweenInteractingPartonsValence->Fill(partonPairDist);
            else // all other cases
                fHistMinDistBetweenInteractingPartonsSoft->Fill(partonPairDist);

            fNumberOfStrings++;
        }
    }
    else if (fFlagPartonDestiny == 1) // May 2018: TRY WQM (it was "wrong" - misinterpreted)
    {
        //
    ...
    else if (fFlagPartonDestiny == 2) // Feb 2026: "rescatter" each parton on the "incoming" partons
    {
        for (int i = 0; i < fA->nPartons; i++)
        {
            for (int j = 0; j < fB->nPartons; j++)
            {
                ...
                float partonPairDist = sqrt(dx * dx + dy * dy);
                if (partonPairDist < fPartonInteractionDistance)
                {
                    // parton-parton rescattering angle vs parton-parton impact par and parton momentum

                    // go to CM system, calc rescattering angles Theta and corresponsing momentum x,y changes in transverse plane
                    float partonPair_E_CM = E_CMS_TMP * sqrt(fA->pxBjorken[i] * fB->pxBjorken[j]);
                    const float kEcmMinCut = 0.5;
                    if (partonPair_E_CM < kEcmMinCut) // important!
                        continue;

                    float QforAlphaS = partonPair_E_CM > kEcmMinCut ? partonPair_E_CM : kEcmMinCut;
                    const float Lambda0 = 0.3; // GeV - to avoid too high alphaS for low Q (important for low Ecm!)
                    const float kPreFactorAlphaS = 0.118 / 0.236 * 12 * TMath::Pi() / 23;
                    // float alphaS = kPreFactorAlphaS / log(QforAlphaS * QforAlphaS / Lambda0 / Lambda0);
                    float alphaS = 0.2;

                    float partonMomCM = partonPair_E_CM / 2; // 2.0;
                    const float kFactorQcdVsQedByHand = 3.;
                    const float hc = 0.197;                                                                               // GeV*fm
                    float ppThetaCM = 2 * atan(alphaS * kFactorQcdVsQedByHand * hc / (2 * partonPairDist * partonMomCM)); // alphaS * 0.197 GeV*fm / b[fm] / p[GeV]

                    double phiA = atan(dy / dx);
                    if (dx < 0)
                        phiA = TMath::Pi() - phiA;
                    FixAngleInTwoPi(phiA);
                    FixAngleInTwoPi(phiA);

                    float dMomX = partonMomCM * sin(ppThetaCM) * cos(phiA);
                    float dMomY = partonMomCM * sin(ppThetaCM) * sin(phiA);

                    // int sign = gRandom->Uniform() > 0.5 ? -1 : +1;

                    // fA->pMomX[i] += sign*dMomX;
                    // fA->pMomY[i] += sign*dMomY;

                    // fB->pMomX[j] -= sign*dMomX;
                    // fB->pMomY[j] -= sign*dMomY;

                    fA->pMomX[i] += (dx > 0 ? +1 : -1) * fabs(dMomX);
                    fA->pMomY[i] += (dy > 0 ? +1 : -1) * fabs(dMomY);

                    fB->pMomX[j] += (dx < 0 ? +1 : -1) * fabs(dMomX);
                    fB->pMomY[j] += (dy < 0 ? +1 : -1) * fabs(dMomY);

                    fA->isWoundedParton[i]++;
                    fB->isWoundedParton[j]++;

                    // int V = gRandom->Uniform() > 0.5 ? 1 : 0;
                    // fA->pMomX[i] += V == 0 ? dMomX : dMomY;
                    // fA->pMomY[i] += V == 0 ? dMomY : dMomX;

                    // fB->pMomX[j] -= V == 0 ? dMomX : dMomY;
                    // fB->pMomY[j] -= V == 0 ? dMomY : dMomX;

                    fHist_partons_individual_kick_dMom_X_Y_QA->Fill(dMomX, dMomY);
                    fHist_partons_kick_theta_QA->Fill(ppThetaCM);
                    fHist_partons_kick_cosTheta_QA->Fill(cos(ppThetaCM));
                    fHist_partons_kick_phi_QA->Fill(phiA);
                    fHist_partons_dx_dy->Fill(dx, dy);

                    // calc nColl
                    if (partonPairDist < 0.5) // !!! important hand-made param!!!
                    {
                        int idNucleonInA = fA->pNid[i];
                        int idNucleonInB = fB->pNid[j];
                        // fA->nuclWounded[idNucleonInA]++;
                        // fB->nuclWounded[idNucleonInB]++;
        ...
        } // end of parton pair loop

        // count wounded partons in nucleus A and B, mark nuclei as wounded
        for (int i = 0; i < fA->nPartons; i++)
        {
            if (fA->isWoundedParton[i] == 0)
                continue;
            ...
            fHist_partons_pT_QA->Fill(pT);
            fHist_partons_pZ_vs_pT_QA->Fill(E_CMS_TMP / 2 * fA->pxBjorken[i], pT);
            fHist_partons_pX_pY_QA->Fill(pX, pY);

            fNWoundsPerParton->Fill(fA->isWoundedParton[i]);
            fHist_partons_x->Fill(fA->pxBjorken[i]);
        ...
        for (int i = 0; i < fB->nPartons; i++)
        {
            ...
            float pT = sqrt(pX * pX + pY * pY);
            fHist_partons_pT_QA->Fill(pT);
            fHist_partons_pZ_vs_pT_QA->Fill(E_CMS_TMP / 2 * fB->pxBjorken[i], pT);
            fHist_partons_pX_pY_QA->Fill(pX, pY);

            fNWoundsPerParton->Fill(fB->isWoundedParton[i]);
            fHist_partons_x->Fill(fB->pxBjorken[i]);
        ...
        ///// QA for Nucleons (Feb 2026)

        for (int iP = 0; iP < fA->nNucleons; iP++)
        {
            fNWoundsPerNucleon->Fill(fA->nuclWounded[iP]);
        }
        for (int iP = 0; iP < fB->nNucleons; iP++)
        {
            fNWoundsPerNucleon->Fill(fB->nuclWounded[iP]);
        }

    } // end of Feb 2026 rescattering of partons
    ...
    if (fNumberOfStrings > 0)
        // if (fNumberOfStrings > 2) // more than 2 strings, Feb 2026
...
void NucleiCollision::drawStatisticHists()
{
    ...
    if (1)
    {
        ...
        fHistNpartonsPerNucleon->DrawCopy();
    ...
    {
        // Feb 2026
        fHist_partons_pT_QA->Write();
        fHist_partons_pZ_vs_pT_QA->Write();
        fHist_partons_pX_pY_QA->Write();

        fNWoundsPerNucleon->Write();
        fNWoundsPerParton->Write();

        fHist_partons_x->Write();
    }
    ...
    if (fFlagUsePartonPDFs)
    {
        ...
        TCanvas *canv_partonKine_QA = new TCanvas("canv_partonKine_QA", "canv_partonKine_QA", 20, 20, 1400, 800);
        canv_partonKine_QA->Divide(4, 3);
        canv_partonKine_QA->cd(1)->SetLogy();
        fHist_partons_pT_QA->DrawCopy();
        canv_partonKine_QA->cd(2)->SetLogz();
        fHist_partons_pZ_vs_pT_QA->DrawCopy("colz");
        fHist_partons_pZ_vs_pT_QA->ProfileX()->DrawCopy("same");
        canv_partonKine_QA->cd(3)->SetLogz();
        fHist_partons_pX_pY_QA->DrawCopy("colz");
        canv_partonKine_QA->cd(4)->SetLogz();
        fHist_partons_individual_kick_dMom_X_Y_QA->DrawCopy("colz");
        canv_partonKine_QA->cd(5)->SetLogz();
        fHist_partons_dx_dy->DrawCopy("colz");

        canv_partonKine_QA->cd(6)->SetLogy();
        fHist_partons_kick_theta_QA->DrawCopy("colz");
        canv_partonKine_QA->cd(7)->SetLogy();
        fHist_partons_kick_cosTheta_QA->DrawCopy("colz");
        canv_partonKine_QA->cd(8);
        fHist_partons_kick_phi_QA->DrawCopy("colz");

        canv_partonKine_QA->cd(9);
        fNWoundsPerParton->DrawCopy();

        canv_partonKine_QA->cd(10);
        fNWoundsPerNucleon->DrawCopy();

        canv_partonKine_QA->cd(11);
        fHist_partons_x->DrawCopy();
    }
}
...
void NucleiCollision::drawPartons()
{
    ...
    {
        elNucl[iP] = new TEllipse(0.5 + fVisNucleusRadiusNucleus * fA->nX[iP] / fNucleusRadius, 0.5 + fVisNucleusRadiusNucleus * fA->nY[iP] / fNucleusRadius, rVisNucleon, rVisNucleon);
        elNucl[iP]->SetLineColor(kGray);
        elNucl[iP]->Draw();
        //        hist2DNuclear->Fill( fX1[iP], fY1[iP] );

        elNucl2[iP] = new TEllipse(0.5 + fVisNucleusRadiusNucleus * fB->nX[iP] / fNucleusRadius, 0.5 + fVisNucleusRadiusNucleus * fB->nY[iP] / fNucleusRadius, rVisNucleon, rVisNucleon);
    ...
    for (int iP = 0; iP < fA->nPartons; iP++)
    {
        elPoints[iP] = new TEllipse(0.5 + fVisNucleusRadiusNucleus * fA->pX[iP] / fNucleusRadius, 0.5 + fVisNucleusRadiusNucleus * fA->pY[iP] / fNucleusRadius, rVisParton, rVisParton);
    ...
    for (int iP = 0; iP < fB->nPartons; iP++)
    {
        ...
        elPoints2[iP] = new TEllipse(0.5 + fVisNucleusRadiusNucleus * fB->pX[iP] / fNucleusRadius, 0.5 + fVisNucleusRadiusNucleus * fB->pY[iP] / fNucleusRadius, rVisParton, rVisParton);
...
void NucleiCollision::drawStrings()
{
    ...
    for (int iP = 0; iP < fNumberOfStrings; iP++)
    {
        elPointsStrings[iP] = new TEllipse(0.5 + fVisNucleusRadiusNucleus * fXstring[iP] / fNucleusRadius, 0.5 + fVisNucleusRadiusNucleus * fYstring[iP] / fNucleusRadius, rVisString, rVisString);
