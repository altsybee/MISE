...
struct Nucleus
{
    ...
    int *nuclWounded; // array of flags for wounded nucleons // March 2026: can we just use pBusy? (or vice versa - remove pBusy)
    ...
    float *pxBjorken; // parton mom x fraction // ADDED Jan 2023

    // May 2018: for WQM mode:
    int *isWoundedParton;

    // Feb 2026: adding rescattering of partons in transverse plane
    float *pMomX;
    float *pMomY;
    ...
    Nucleus(int nNucleons_, int maxNofPartons_)
    {
        ...
        pxBjorken = new float[maxNofPartons];

        // Feb 2026: adding rescattering of partons in transverse plane
        pMomX = new float[maxNofPartons];
        pMomY = new float[maxNofPartons];
    }
    virtual ~Nucleus()
    {
        delete[] nX;
...
public:
    ...
    void setImpactParameterRange(float min, float max)
    {
        fImpactParameter_rangeMin = min;
        fImpactParameter_rangeMax = max;
    }
    ...
    void setFlagPartonDestiny(int flag) { fFlagPartonDestiny = flag; }
    ...
    double getNu() { return (2. * fNcollisions) / fNparticipants; }
...
private:
    ...
    int fFlagPartonDestiny;  // by default - "burn-out" partons and do not allow them to interact again
    ...
    float *fDistanceBetweenPartonsForString; // what was the distance between partons which gave this string
    ...
    // Feb 2026
    TH1D *fHist_partons_pT_QA;
    TH2D *fHist_partons_pZ_vs_pT_QA;
    TH2D *fHist_partons_pX_pY_QA;
    TH2D *fHist_partons_individual_kick_dMom_X_Y_QA;
    TH2D *fHist_partons_dx_dy;
    TH1D *fHist_partons_kick_theta_QA;
    TH1D *fHist_partons_kick_cosTheta_QA;
    TH1D *fHist_partons_kick_phi_QA;

    TH1D *fHist_partons_x;

    TH1D *fNWoundsPerNucleon;
    TH1D *fNWoundsPerParton;
