...
R__LOAD_LIBRARY(StringGeneration/NucleiCollision_cpp.so)
R__LOAD_LIBRARY(StringGeneration/DistanceEntry_cpp.so)
R__LOAD_LIBRARY(StringGeneration/MinDistanceFinder_cpp.so)
R__LOAD_LIBRARY(ManagerNucleiCollisions_cpp.so)

void launchNucleiCollisionGenerator(int nEvents = 5e3,
                                    int flagMBorFixedB = 2, // 0,1,2 - MB event, precise b, b in range
                                    float bImpact = 0,
                                    double partonInteractionDist = 2., //0.6,//0.25, //1., //2, //0.25,
                                    //        , double stringInteractionRadius = 0.4
                                    //        , double strIntDistFractionParA = 10.
                                    double meanPartonsInNucleon = 6,
                                    double nucleonGaussianRadius = 0.8, //0.4,
                                    ...
                                    int setFlagPartonDestiny = 2, // Feb 2026; was: 0 or 1
                                    int drawMode = 0)
{
        ...
        TString strOutputDirName_NucleiCollision = "outputs_NucleiCollision_REWAKE_2024";
        ...
        if (1)
        {
                ...
                d->setOutputDirectoryName(strOutputDirName_NucleiCollision.Data());
                ...
                {
                        if (int(bImpact) == 0)
                                d->setImpactParameterRange(0, 3.50); // 0-5%
                        else if (int(bImpact) == 1)
                                d->setImpactParameterRange(3.5, 4.94); // 5-10%
                        else if (int(bImpact) == 2)
                                d->setImpactParameterRange(4.94, 6.98); // 10-20%
                        else if (int(bImpact) == 3)
                                d->setImpactParameterRange(6.98, 8.55); // 20-30%
                        else if (int(bImpact) == 4)
                                d->setImpactParameterRange(8.55, 9.88); // 30-40%
                        else if (int(bImpact) == 5)
                                d->setImpactParameterRange(9.88, 11.04); // 40-50%
                        else if (int(bImpact) == 6)
                                d->setImpactParameterRange(11.04, 12.09); // 50-60%
                        else if (int(bImpact) == 7)
                                d->setImpactParameterRange(12.09, 13.05); // 60-70%
                        else if (int(bImpact) == 8)
                                d->setImpactParameterRange(13.05, 13.97); // 70-80%
                }
                ...
                d->setFlagPartonDestiny(setFlagPartonDestiny);

                d->setFlagUsePartonPDFs(true); // Jan 2023; !!! it overwrites the flagUsePoissonianNpartonsFluctuations!
                ...
                evMan.setFillTree( false ); // placed back on March 11, 2026
                evMan.setOutputDirectoryName(strOutputDirName_NucleiCollision.Data()); // strOutputDirName_EventManager );
